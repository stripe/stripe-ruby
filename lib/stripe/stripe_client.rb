...
module Stripe
  class StripeClient
    ...
    def parse_event_notification(payload, sig_header, secret, tolerance: Webhook::DEFAULT_TOLERANCE)
      ...
      if parsed[:type].to_s.start_with?("v2.extend.objects.object_record") && parsed[:related_object] && parsed[:related_object][:type]
        parsed[:type] = parsed[:type].sub(/\Av2\.extend\.objects\.object_record/, parsed[:related_object][:type].to_s)
      end
