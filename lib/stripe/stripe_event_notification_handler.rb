...
module Stripe
  ...
  class StripeEventNotificationHandlerBase
    ...
    def on_v2_signals_account_evaluation_complete(&handler)
      raise ArgumentError, "Block required to register event handler" if handler.nil?

      register("v2.signals.account_evaluation.complete", &handler)
    end
    ...
    def on_v2_signals_account_signal_fraudulent_website_ready(&handler)
      raise ArgumentError, "Block required to register event handler" if handler.nil?

      register("v2.signals.account_signal.fraudulent_website_ready", &handler)
    end
