  ...
  class ApiResourceTest < Test::Unit::TestCase
    ...
    context ".custom_method" do
      should "call to an RPC-style method" do
        stub_request(:post, "#{EwStripe.api_base}/v1/custom_methods/ch_123/my_method")
    ...
    should "creating a new APIResource should not fetch over the network" do
      EwStripe::Customer.new("someid")
      assert_not_requested :get, %r{#{EwStripe.api_base}/.*}
    end

    should "creating a new APIResource from a hash should not fetch over the network" do
      EwStripe::Customer.construct_from(id: "somecustomer",
    ...
    should "setting an attribute should not cause a network request" do
      c = EwStripe::Customer.new("cus_123")
      c.card = { id: "somecard", object: "card" }
      assert_not_requested :get, %r{#{EwStripe.api_base}/.*}
      assert_not_requested :post, %r{#{EwStripe.api_base}/.*}
    end
    ...
    should "not specifying api credentials should raise an exception" do
      ...
      assert_raises EwStripe::AuthenticationError do
        EwStripe::Customer.new("cus_123").refresh
      end
    end

    should "using a nil api key should raise an exception" do
      assert_raises TypeError do
        EwStripe::Customer.list({}, nil)
      end
      assert_raises TypeError do
        EwStripe::Customer.list({}, api_key: nil)
      end
    end

    should "specifying api credentials containing whitespace should raise an exception" do
      EwStripe.api_key = "key "
      assert_raises EwStripe::AuthenticationError do
        EwStripe::Customer.new("cus_123").refresh
      end
    end
    ...
    should "send expand on fetch properly" do
      ...
      EwStripe::Charge.retrieve(id: "ch_123", expand: [:customer])
    end
    ...
    should "preserve expand across refreshes" do
      ...
      ch = EwStripe::Charge.retrieve(id: "ch_123", expand: [:customer])
      ch.refresh
    end
    ...
    should "send expand when fetching through ListObject" do
      ...
      stub_request(:get, "#{EwStripe.api_base}/v1/charges/ch_123/refunds/re_123")
    ...
    context "when specifying per-object credentials" do
      context "with no global API key set" do
        should "use the per-object credential when creating" do
          ...
          EwStripe::Charge.create({ source: "tok_visa" },
                                "sk_test_local")
        end
      end
      ...
      context "with a global API key set" do
        ...
        should "use the per-object credential when retrieving and making other calls" do
          stub_request(:get, "#{EwStripe.api_base}/v1/charges/ch_123")
            .with(headers: { "Authorization" => "Bearer sk_test_local" })
            .to_return(body: JSON.generate(charge_fixture))
          stub_request(:post, "#{EwStripe.api_base}/v1/charges/ch_123/refunds")
            .with(headers: { "Authorization" => "Bearer sk_test_local" })
            .to_return(body: "{}")

          ch = EwStripe::Charge.retrieve("ch_123", "sk_test_local")
          ch.refunds.create
        end
      end
    end
    ...
    context "with valid credentials" do
      should "urlencode values in GET params" do
        ...
        charges = EwStripe::Charge.list(customer: "test customer").data
      ...
      should "construct URL properly with base query parameters" do
        stub_request(:get, "#{EwStripe.api_base}/v1/charges")
          ...
          .to_return(body: JSON.generate(data: [charge_fixture],
                                         url: "/v1/charges"))
        charges = EwStripe::Charge.list(customer: "cus_123")
      ...
      should "setting a nil value for a param should exclude that param from the request" do
        ...
        EwStripe::Charge.list(count: nil, offset: 5, sad: false)

        stub_request(:post, "#{EwStripe.api_base}/v1/charges")
          .with(body: { "amount" => "50", "currency" => "usd" })
          .to_return(body: JSON.generate(count: 1, data: [charge_fixture]))
        EwStripe::Charge.create(amount: 50, currency: "usd", card: { number: nil })
      end
      ...
      should "not trigger a warning if a known opt, such as idempotency_key, is in opts" do
        ...
        $stderr = StringIO.new
        begin
          EwStripe::Charge.create({ amount: 100, currency: "usd", card: "sc_token" }, idempotency_key: "12345")
      ...
      should "error if the params is not a Hash" do
        stub_request(:post, "#{EwStripe.api_base}/v1/charges/ch_123/capture")
          .to_return(body: JSON.generate(charge_fixture))

        e = assert_raises(ArgumentError) { EwStripe::Charge.capture("ch_123", "sk_test_secret") }
      ...
      should "allow making a request with params set to nil" do
        ...
        EwStripe::Charge.capture("ch_123", nil, "sk_test_secret")
      end
      ...
      should "error if a user-specified opt is given a non-nil non-string value" do
        ...
        EwStripe::Charge.create({ amount: 100, currency: "usd" }, {})
        EwStripe::Charge.create({ amount: 100, currency: "usd" }, idempotency_key: "12345")

        # Errors on a non-string.
        e = assert_raises(ArgumentError) do
          EwStripe::Charge.create({ amount: 100, currency: "usd" }, idempotency_key: :foo)
        end
      ...
      should "requesting with a unicode ID should result in a request" do
        stub_request(:get, "#{EwStripe.api_base}/v1/customers/%E2%98%83")
          .to_return(body: JSON.generate(make_missing_id_error), status: 404)
        c = EwStripe::Customer.new("☃")
        assert_raises(EwStripe::InvalidRequestError) { c.refresh }
      end
      ...
      should "making a POST request with parameters should have a body and no query string" do
        stub_request(:post, "#{EwStripe.api_base}/v1/charges")
          ...
          .to_return(body: JSON.generate(charge_fixture))
        EwStripe::Charge.create(amount: 100, currency: "usd", card: "sc_token")
      end

      should "loading an object should issue a GET request" do
        stub_request(:get, "#{EwStripe.api_base}/v1/charges/ch_123")
          .to_return(body: JSON.generate(charge_fixture))
        c = EwStripe::Charge.new("ch_123")
        c.refresh
      end
      ...
      should "updating an object should issue a POST request with only the changed properties" do
        stub_request(:post, "#{EwStripe.api_base}/v1/customers/cus_123")
          .with(body: { "description" => "another_mn" })
          .to_return(body: JSON.generate(customer_fixture))
        c = EwStripe::Customer.construct_from(customer_fixture)
      ...
      should "updating should use the supplied api_key" do
        stub_request(:post, "#{EwStripe.api_base}/v1/customers")
          .with(headers: { "Authorization" => "Bearer sk_test_local" })
          .to_return(body: JSON.generate(customer_fixture))
        c = EwStripe::Customer.new
      ...
      should "deleting should send no props and result in an object that has no props other deleted" do
        stub_request(:delete, "#{EwStripe.api_base}/v1/customers/cus_123")
      ...
      should "loading all of an APIResource should return an array of recursively instantiated objects" do
        stub_request(:get, "#{EwStripe.api_base}/v1/charges")
          .to_return(body: JSON.generate(data: [charge_fixture]))
        charges = EwStripe::Charge.list.data
        assert charges.is_a? Array
        assert charges[0].is_a? EwStripe::Charge
        assert charges[0].payment_method_details.is_a?(EwStripe::StripeObject)
      end
      ...
      should "passing in a stripe_account header should pass it through on call" do
        stub_request(:get, "#{EwStripe.api_base}/v1/customers/cus_123")
          ...
          .to_return(body: JSON.generate(customer_fixture))
        EwStripe::Customer.retrieve("cus_123", stripe_account: "acct_123")
      end
      ...
      should "passing in a stripe_account header should pass it through on save" do
        stub_request(:get, "#{EwStripe.api_base}/v1/customers/cus_123")
          .with(headers: { "Stripe-Account" => "acct_123" })
          .to_return(body: JSON.generate(customer_fixture))
        c = EwStripe::Customer.retrieve("cus_123", stripe_account: "acct_123")
      ...
      should "add key to nested objects" do
        acct = EwStripe::Account.construct_from(id: "myid",
                                              legal_entity: {
                                                size: "l",
                                                score: 4,
                                                height: 10,
        ...
        stub_request(:post, "#{EwStripe.api_base}/v1/accounts/myid")
      ...
      should "save nothing if nothing changes" do
        acct = EwStripe::Account.construct_from(id: "acct_id",
                                              metadata: {
                                                key: "value",
                                              })

        stub_request(:post, "#{EwStripe.api_base}/v1/accounts/acct_id")
          .with(body: {})
      ...
      should "not save nested API resources" do
        ch = EwStripe::Charge.construct_from(id: "ch_id",
                                           customer: {
                                             object: "customer",
                                             id: "customer_id",
                                           })

        stub_request(:post, "#{EwStripe.api_base}/v1/charges/ch_id")
          .with(body: {})
      ...
      should "correctly handle replaced nested objects" do
        acct = EwStripe::Account.construct_from(
          id: "acct_123",
          company: {
            name: "company_name",
            address: {
        ...
        stub_request(:post, "#{EwStripe.api_base}/v1/accounts/acct_123")
      ...
      should "correctly handle array insertion" do
        ...
        # data decoding, which isn't aware of the EwStripe array encoding that we
        # use here.
        stub_request(:post, "#{EwStripe.api_base}/v1/accounts/myid")
          .with(body: { legal_entity: { additional_owners: [{ first_name: "Bob" }] } })
          .to_return(body: JSON.generate("id" => "myid"))
      ...
      should "should create a new resource when an object without an id is saved" do
        account = EwStripe::Account.construct_from(id: nil,
                                                 display_name: nil)

        stub_request(:post, "#{EwStripe.api_base}/v1/accounts")
    ...
    context "#request_stripe_object" do
      ...
      setup do
        Util.instance_variable_set(
          :@object_classes,
          EwStripe::ObjectTypes.object_names_to_classes.merge(
            "hello" => HelloTestAPIResource
          )
        )
      end
      teardown do
        Util.class.instance_variable_set(:@object_classes, EwStripe::ObjectTypes.object_names_to_classes)
      end

      should "make requests appropriately" do
        stub_request(:post, "#{EwStripe.api_base}/v1/hellos/hi_123/say")
    ...
    context "#request_stream" do
      ...
      should "supports requesting with a block" do
        stub_request(:get, "#{EwStripe.api_base}/v1/streams/hi_123/read")
