# frozen_string_literal: true

require ::File.expand_path("../test_helper", __dir__)

module EwStripe
  class ReversalTest < Test::Unit::TestCase
    setup do
      @transfer = EwStripe::Transfer.retrieve("tr_123")
    end

    should "be listable" do
      reversals = @transfer.reversals.list
      assert_requested :get,
                       "#{EwStripe.api_base}/v1/transfers/#{@transfer.id}/reversals"
      assert reversals.data.is_a?(Array)
      assert reversals.data[0].is_a?(EwStripe::Reversal)
    end

    should "be retrievable" do
      reversal = @transfer.reversals.retrieve("trr_123")
      assert_requested :get,
                       "#{EwStripe.api_base}/v1/transfers/#{@transfer.id}/reversals/trr_123"
      assert reversal.is_a?(EwStripe::Reversal)
    end

    should "be creatable" do
      reversal = @transfer.reversals.create(
        amount: 100
      )
      assert_requested :post,
                       "#{EwStripe.api_base}/v1/transfers/#{@transfer.id}/reversals"
      assert reversal.is_a?(EwStripe::Reversal)
    end

    should "be saveable" do
      reversal = @transfer.reversals.retrieve("trr_123")
      reversal.metadata["key"] = "value"
      reversal.save
      assert_requested :post,
                       "#{EwStripe.api_base}/v1/transfers/#{reversal.transfer}/reversals/#{reversal.id}"
    end
  end
end
